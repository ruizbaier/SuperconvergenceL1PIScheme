...
fileO = XDMFFile("outputs/fullProblem-cv-3D.xdmf")
fileO.parameters["functions_share_mesh"] = False
...
#K = as_tensor(((2, 0, 0), (0, 0.1, 0), (0, 0, 1.0)))


u_str = 't^(2+alpha) * cos(pi*x) * sin(pi*y)*sin(pi*z)'
Gamma_ratio = math.gamma(3 + alpha) / math.gamma(3)
integral_factor = math.gamma(3 + alpha) / math.gamma(lambda_r + 3 + alpha)

Dt_alpha_u_str = 'Gamma_ratio * pow(t,2) * cos(pi*x) * sin(pi*y) * sin(pi*z)'
I_lambda_u_str = 'integral_factor * t^(lambda_r+2+alpha) * cos(pi*x) * sin(pi*y) * sin(pi*z)'
...
for nk in range(nkmax):
    ...
    for n in range(1, nsteps + 1):
        ...
        f_ex = Dt_alpha_u_ex - div(grad(u_ex)) - gamma_coeff * I_lambda_u_ex
        ...
        # Weak forms
        auv = (C_alpha - coeff_curr) * u * v * dx + dot(grad(u), grad(v)) * dx
        Fv = (f_ex + C_alpha * u_history[n - 1] - history_sum - gamma_coeff * integral_rhs) * v * dx
        
        bcU = DirichletBC(Vh, u_ex, 'on_boundary')
        u_curr = Function(Vh)
        solve(auv == Fv, u_curr, bcU, solver_parameters={'linear_solver': 'mumps'})
    ...
    u_h.rename("u", "u"); fileO.write(u_h, 1.0*nk)
